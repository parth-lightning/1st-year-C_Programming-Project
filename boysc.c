...
// Save data to file
void saveData() {
    FILE *file = fopen("students.dat", "wb");//wb for binary file
    if (file) {
        fwrite(&studentCount, sizeof(int), 1, file);// fwrite function to write binary//
        fwrite(students, sizeof(Student), studentCount, file);
        fclose(file);
    }// here yoou will notice not use for loop because we are calling it whenwe are writting something
}
...
// Teacher Section
void  teacherSection() {
    while (1) {// while 1 because it will run till return  statement 
        printf("\n--- Teacher Section ---\n");
        ...
        if (choice == 1) {
            if (studentCount < 100) {
                ...
                getchar();
                scanf("%[^\n]s", students[studentCount].name);//student count same like i in for loop
                ...
                for (int i = 0; i < students[studentCount].numSubjects; i++) {
                    ...
                    getchar();
                    scanf("%[^\n]s", students[studentCount].subjectNames[i]);// this%[^\n]s is used for space
                ...
                studentCount++;// after this studentcount will increase
                saveData();// here we call the function
                printf("Student added successfully!\n");
            } else {
                printf("Student limit reached!\n");
            }
        } else if (choice == 2) {// for deletion
            int prn;
            ...
            for (int i = 0; i < studentCount; i++) {
                if (students[i].prn == prn) {
                    found = 1;
                    for (int j = i; j < studentCount - 1; j++) {// it left shifts the data while deletion
                        students[j] = students[j + 1];
                    }
                    studentCount--;// and decrease the number of studentcount
                    saveData();// again we call that function
            ...
            return ;//here we put return so it will stop and will redirect to login where we call this function
        } else {
...
void studentSection(int prn) {
    ...
    for (int i = 0; i < studentCount; i++) {
        if (students[i].prn == prn) {
            ...
            return;// return to login function where we call it 
        }
    }
    if (found==0) {
        printf("Student with PRN %d not found!\n", prn);
...
void verifyRole(int prn) {
    ...
    if (firstDigit == 1) {
        studentSection(prn);
    } else if (teacher == 24567894) {//this will be the only prn through which teacher can login
        teacherSection();
...
// Login function
void login() {
    while (1) {// same till return it will run 
        int prn, password;
...
// Main function
int main() {
    loadData();// first we call this function load the data means to read
    login();// then login function
    return 0;
}
